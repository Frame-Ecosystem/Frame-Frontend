...
class ServiceService {
  async getAll(): Promise<Service[]> {
    try {
      ...
      console.warn('Unexpected response format for /v1/admin/services:', response)
  ...
  async getById(id: string): Promise<Service | null> {
    try {
      const response = await apiClient.get<any>(`/v1/admin/services/${id}`)
      // Handle different response formats
    ...
    try {
      ...
      if (createdService) {
        ...
        return {
          ...
          categoryId: typeof createdService.categoryId === 'object' ? createdService.categoryId._id || createdService.categoryId.id : createdService.categoryId
        } as Service
      }
  ...
  async update(id: string, service: Partial<Omit<Service, 'id' | 'createdAt' | 'updatedAt'>>): Promise<Service> {
    try {
      const response = await apiClient.put<any>(`/v1/admin/services/${id}`, service)
      // Handle different response formats
      ...
      if (updatedService) {
        ...
        return {
          ...
          categoryId: typeof updatedService.categoryId === 'object' ? updatedService.categoryId._id || updatedService.categoryId.id : updatedService.categoryId
        } as Service
      }
