...
export async function PUT(
  ...
  try {
    ...
    if (body.name) {
      ...
      const duplicate = mockServices.some(s => s.id !== id && s.name.toLowerCase() === newName.toLowerCase())
      if (duplicate) {
    ...
    const updatedService = {
      ...
      basePrice: body.basePrice ? parseFloat(body.basePrice) : mockServices[serviceIndex].basePrice,
      estimatedDuration: body.estimatedDuration ? parseInt(body.estimatedDuration) : mockServices[serviceIndex].estimatedDuration,
      updatedAt: new Date().toISOString()
    }
