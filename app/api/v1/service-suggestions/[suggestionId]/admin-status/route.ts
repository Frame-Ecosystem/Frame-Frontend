...
const ADMIN_CATEGORIES_FILE = path.join(
  process.cwd(),
  "app",
  "api",
  "v1",
...
async function loadData() {
  ...
  try {
    const adminCategoriesData = await fs.readFile(
      ADMIN_CATEGORIES_FILE,
      "utf-8",
    )
    mockAdminCategories = JSON.parse(adminCategoriesData)
  } catch {
    mockAdminCategories = []
  }
}
...
// PATCH /api/v1/service-suggestions/:suggestionId/admin-status
export async function PATCH(
  ...
  try {
    ...
    if (action === "reject") {
      ...
      if (body.adminNote) {
        suggestion.adminComment = body.adminNote
      }
      ...
      // Approval with service creation
      const categoryId = body.categoryId ? String(body.categoryId).trim() : ""
      if (!categoryId) {
        return NextResponse.json(
          {
            message: "categoryId is required for approval",
            code: "BAD_REQUEST",
          },
          { status: 400 },
        )
      ...
      const categoryExists = mockAdminCategories.some(
        (cat) => cat.id === categoryId,
      )
      if (!categoryExists) {
        return NextResponse.json(
          { message: "Selected category does not exist", code: "BAD_REQUEST" },
          { status: 400 },
        )
      }
