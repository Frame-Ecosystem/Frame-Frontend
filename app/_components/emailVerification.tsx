...
export function EmailVerification({ email, isVerified = false }: EmailVerificationProps) {
  ...
  const handleSendCode = async () => {
    ...
    try {
      ...
      setMessage({ type: 'error', text: error instanceof Error ? error.message : 'Failed to send verification code' })
    } finally {
      setIsSending(false)
    }
  }
    ...
    <>
      {isVerified ? (
        <Button
          ...
          variant="outline"
          size="sm"
          className="flex items-center gap-1 bg-green-50 border-green-200 text-green-700 hover:bg-green-50 hover:text-green-700 cursor-not-allowed "
        >
      ...
      {showCodeInput && (
        <div className="space-y-3 p-3 bg-muted/50 rounded-lg mt-3">
          <div>
      ...
      {message && (
        <div className={`flex items-center gap-2 text-sm p-2 rounded mt-2 ${
