...
import { getUserDisplayName, getUserInitials } from "../_services/auth.service"
...
const UserSession = ({ compact }: { compact?: boolean } = {}) => {
  ...
  const userButton = (
    ...
    >
      {isLoggedIn ? (
        <Avatar className={`${compact ? 'h-8 w-8' : 'h-12 w-12'} ring-1 ring-primary/20`}>
          {user?.profileImage && (
            <AvatarImage src={typeof user.profileImage === 'string' ? user.profileImage : user.profileImage.url} alt={getUserDisplayName(user)} />
          )}
          <AvatarFallback className={`bg-primary/10 text-primary font-medium ${compact ? 'text-xs' : 'text-sm'}`}>
            {getUserInitials(user)}
          </AvatarFallback>
        </Avatar>
      ) : (
        <div className={`rounded-full border border-border p-4 flex items-center justify-center ${compact ? 'h-10 w-10 p-1' : 'h-12 w-12 p-1'}`}>
          <UserIcon className={`${compact ? 'h-4 w-4' : 'h-8 w-8'} text-muted-foreground`} />
        </div>
      )}
    </Button>
  )
      ...
      <>
        <Popover open={popoverOpen} onOpenChange={setPopoverOpen}>
          ...
          <PopoverContent className="mt-2 w-72 p-0" align="end">
            <UserInfo user={user} onAddAccount={handleAddAccount} onClose={() => setPopoverOpen(false)} />
          </PopoverContent>
        </Popover>
